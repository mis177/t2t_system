  ...
  void initSensors() {
    ...
    _streamSubscriptions.add(
      userAccelerometerEventStream().listen((event) {
        ...
        double accelerometeValue = 0;
        if (phoneIsLayingFlat) {
          accelerometeValue = event.y;
        } else {
          accelerometeValue = event.z;
        }

        if (accelerometeValue.abs() > 0.08) {
          movementSpeed = (accelerometeValue.abs());
          notifyListeners();
    ...
    _streamSubscriptions.add(
      gyroscopeEventStream(samplingPeriod: Durations.extralong4)
          .listen((event) {
